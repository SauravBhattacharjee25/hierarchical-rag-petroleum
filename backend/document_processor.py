...
def extract_text_from_pdf(filepath: str) -> str:
    ...
    if PyPDF2 is None:
        if config.DEBUG_MODE:
            print(f"  ⚠️  PyPDF2 not installed; skipping PDF: {Path(filepath).name}")
        return ""
...
def extract_text_from_docx(filepath: str) -> str:
    ...
    if Document is None:
        if config.DEBUG_MODE:
            print(f"  ⚠️  python-docx not installed; skipping DOCX: {Path(filepath).name}")
        return ""
...
def extract_text_from_xlsx(filepath: str) -> str:
    ...
    if openpyxl is None:
        if config.DEBUG_MODE:
            print(f"  ⚠️  openpyxl not installed; skipping XLSX: {Path(filepath).name}")
        return ""
...
def extract_images_from_pdf(filepath: str) -> str:
    ...
    if PyPDF2 is None:
        if config.DEBUG_MODE:
            print(f"  ⚠️  PyPDF2 not installed; skipping PDF image extraction: {Path(filepath).name}")
        return ""
